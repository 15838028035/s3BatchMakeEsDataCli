...
public class ThreadPoolMonitor extends ThreadPoolExecutor {
    ...
    public void shutdown() {
    	...
    	GLogger.info("{} Going to shutdown. Executed tasks: {}, Running tasks: {}, Pending tasks: {}",
    ...
    public List<Runnable> shutdownNow() {
        // 统计已执行任务、正在执行任务、未执行任务数量
    	GLogger.info("{} Going to immediately shutdown. Executed tasks: {}, Running tasks: {}, Pending tasks: {}",
                this.poolName, this.getCompletedTaskCount(), this.getActiveCount(), this.getQueue().size());
    ...
    protected void afterExecute(Runnable r, Throwable t) {
       ...
       if(startDate == null) {
    	   GLogger.error("多线程监控线程池开始时间为空,线程名称:{})", t);
       }
     ...
     */
     ...
     public void execute(Runnable task) {
         super.execute(ThreadPoolMDCFilter.wrap(task, new HashMap<>())); 
     }
    
    @Override
    public <T> Future<T> submit(Callable<T> task) {
        return super.submit(ThreadPoolMDCFilter.wrap(task, new HashMap<>()));
    }
